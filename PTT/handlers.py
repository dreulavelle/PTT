...
def add_defaults(parser: Parser):
    ...
    parser.add_handler("date", regex.compile(r"(?:\W)(\[?\]?(?:[0][1-9]|[12][0-9]|3[0-9])([. \-/\\])(?:0[1-9]|1[012])\2(?:0[1-9]|[12][0-9])[\])]?)(?:\W|$)"), date("YY MM DD"), {"remove": True})
    ...
    parser.add_handler("codec", regex.compile(r"\b\W264\W\b"), value("avc"), {"remove": True, "skipIfAlreadyFound": True, "skipFromTitle": True})
    parser.add_handler("codec", regex.compile(r"\b\W265\W\b"), value("hevc"), {"remove": True, "skipIfAlreadyFound": True, "skipFromTitle": True})
    ...
    parser.add_handler("channels", regex.compile(r"\b(?:24-bit\s)?stereo\b", regex.IGNORECASE), uniq_concat(value("stereo")), {"remove": False, "skipIfAlreadyFound": False})
    ...
    parser.add_handler("complete", regex.compile(r"(?:\bthe\W)?(?:\bcomplete\b|\bfull\b|\ball\b)\b.*\b(?:series|seasons|collection|episodes|set|pack|movies)\b", regex.IGNORECASE), boolean)
    parser.add_handler("complete", regex.compile(r"(Top\W+)?\d+\W+(movies?|series|seasons?)\W+Collection", regex.IGNORECASE), boolean, {"remove": True})
    ...
    parser.add_handler("episodes", regex.compile(r"Серии:\s+(\d+)\s+(?:of|из|iz)\s+\d+\b", regex.IGNORECASE), range_x_of_y_func)
    ...
    parser.add_handler("languages", regex.compile(r"\b(PLDUB|PLSUB|DUBPL|DubbingPL|LekPL|LektorPL)\b", regex.IGNORECASE), uniq_concat(value("pl")), {"remove": True, "skipIfAlreadyFound": False})
