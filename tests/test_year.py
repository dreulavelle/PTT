...
@pytest.mark.parametrize("release_name, expected_year, expect_year_presence", [
    ...
    ("Harry Potter All Movies Collection 2001-2011 720p Dual KartiKing", "2001-2011", True),
    ("Empty Nest Season 1 (1988 - 89) fiveofseven", "1988-1989", True),
...
def test_year_detection(parser, release_name, expected_year, expect_year_presence):
    ...
    if expect_year_presence:
        assert "year" in result and result["year"] == expected_year, f"Expected year to be {expected_year} for {release_name}"
    else:
